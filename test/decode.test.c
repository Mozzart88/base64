...
void test_decode(void)
{
	...
	const char test_data[tests_count][45] = {
		"SGVsbG8sIFdvcmxkIQ==",
	...
	const t_data expected[tests_count][data_len] = {
	...
	t_data actual = {0, NULL};
	uint8_t i = 0;

	if ((actual.data = (uint8_t*)malloc(sizeof(uint8_t)*45)) == NULL)
		return;
	write(STDOUT_FILENO, "Test decode: ", 13);
	while (i < tests_count)
	{
		t_data exp = *(expected[i]);
		actual.len = base64_decode(test_data[i], &(actual.data));
		if (actual.len == exp.len && memcmp(actual.data, exp.data, actual.len) == 0)
		{
